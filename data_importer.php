...
<?PHP
	...
	if (isset($_FILES['filexls']['name']) && $_FILES['filexls']['name'] != '') {
		...
		//行之1: 【種別☆レアリティ】名称　状態　Lv　HP　ATK　HP+ATK　CP　スキル効果　性別　フォース
		//行之2: 限突　Lv　HP　ATK　HP+ATK　CP
		$type = 0;
		$arrType1 = array();
		$cost = 2;
		$sql = 'INSERT IGNORE INTO `ma`.`cardbase` (`jname`, `cost`, `maxlv1`, `maxlv2`, `skill`, `rare`, `gender`, `race`, `force`) VALUES (:jname, :cost, :maxlv1, :maxlv2, :skill, :rare, :gender, :race, :force);';
		...
		for($i; $i <= $highestRow; $i++) {
				...
				*/
				...
				if(ord($j)-65 == 0){
					...
					if(preg_match('/【(.+?)☆(\d?)】(.+)/', $val, $matchs)){
						...
						$arrType1['race'] = $matchs[1];
						$arrType1['rare'] = $matchs[2];
						$arrType1['jname'] = $matchs[3];
			...
			if($arrType1['jname']){
				if($lv2 || $lv2 == 0){
					//INSERT data
					$params = array(':jname'=>$arrType1['jname'], ':cost'=>$cost, ':maxlv1'=>$arrType1['lv1'], ':maxlv2'=>$lv2, ':skill'=>0, ':rare'=>$arrType1['rare'], ':gender'=>$arrType1['gender'], ':race'=>$arrType1['race'], ':force'=>$arrType1['force']);
					$sth->execute($params);
					if($sth->rowCount() >0)
			...
			} else echo '<br><font color="red">line '.$i.' 無抓取資料!!($rare:'.$rare.' $jname:'.$jname.' $lv1:'.$lv1.' $gender:'.$gender.' $race:'.$race.' $force:'.$force.')</font><br>';
			//echo '<br>-----------------------<br>';
		}
	}
